...
#include <set>
#include <vector>
#define LABEL_NALE_TXT_PATH "./model/coco_105_labels_list.txt"
...
}void matmul_by_npu_uint8(std::vector<float> &A_input, float *B_input, uint8_t *C_input, int ROWS_A, int COLS_A, int COLS_B, rknn_app_context_t *app_ctx) 
{
    int B_layout = 0;
    int AC_layout = 0;
    int32_t M = ROWS_A;
    ...
    info.K = K;
    info.N = N;
    info.type = RKNN_INT8_MM_INT8_TO_INT8;
    ...
    uint8_t *int8Vector_A = new uint8_t[ROWS_A * COLS_A];
    for (int i = 0; i < ROWS_A * COLS_A; ++i)
    {
        int8Vector_A[i] = A_input[i];
    }

    uint8_t *int8Vector_B = new uint8_t[COLS_A * COLS_B];
    for (int i = 0; i < COLS_A * COLS_B; ++i)
    {
        int8Vector_B[i] = B_input[i];
    }
    ...
    {
        C_input[i] = ((uint8_t *)C->virt_addr)[i];
    }
...
{
    std::cout << "cropped_height: " << cropped_height << std::endl;
    std::cout << "cropped_width: " << cropped_width << std::endl;
    std::cout << "ori_in_height: " << ori_in_height << std::endl;
    std::cout << "ori_in_width: " << ori_in_width << std::endl;
...
int post_process(rknn_app_context_t *app_ctx, rknn_output *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold, object_detect_result_list *od_results)
{
    ...
    // matmul_by_npu_uint8(filterSegments_by_nms, proto, matmul_out, ROWS_A, COLS_A, COLS_B, app_ctx);
    timer.tok();
...
char *coco_cls_to_name(int cls_id)
{
    ...
    // printf("\n\n\n\n%dsosososososososososoososos\n\n\n\n", cls_id);
    if (cls_id >= OBJ_CLASS_NUM)
    {
        // printf("\n\n\nsosososososososososoososos\n\n\n\n", cls_id);
        return "null";
    }
...
int resize_image(cv::Mat &src_mat, cv::Mat &dst_mat) {
    int ret = 0;
    int src_width = 2592;
    int src_height = 1944;
    ...
    // Convert Mat to raw buffer (assuming the image is in RGBA format)
    memcpy(src_buf, src_mat.data, src_buf_size);

    // Initialize destination buffer
    memset(dst_buf, 0x80, dst_buf_size);

    // Import the buffers into RGA handles
    ...
    // Wrap the buffers into rga_buffer_t structures
    ...
    // Copy the resized image back into Mat
